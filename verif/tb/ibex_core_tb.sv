...
module ibex_core_tb;
  ...
  task automatic emit(input logic [31:0] insn);
    instr_mem[pc_wr] = insn; $display("[EMIT] %08h -> [%0d]", insn, pc_wr); pc_wr = pc_wr + 1;
  endtask
  ...
  initial begin
    ...
    // Debug: dump first 8 instr_mem words
    for (int _d=0; _d<8; _d++) $display("[IMEM] imem[%0d]=%08h", _d, instr_mem[_d]);
    // Run all test groups
